...
| 저자가 상업까지 허용(permissive) | TIPI | 🟢 | 🟢 (저자가 명시적으로 모든 용도 허용) |
| 연구용 무료 | BFI-44, BFI-2, Saucier, HEXACO-60 | 🟢 | 🔴 (저자 허가 필요) |
...
**저장소 라이선스**: 이 프로젝트 자체(코드·문서·번역·원저작 콘텐츠)는 저장소 최상위 `LICENSE`(CC BY-NC-SA 4.0, 비상업)를 따른다. 이 폴더의 개별 문항은 **각 파일 `_meta.license`가 우선**이며(예: IPIP는 저장소 라이선스와 무관하게 원래부터 퍼블릭 도메인), 저장소 기본 라이선스는 그 위에 "적어도 이만큼은 제한한다"는 하한선 역할만 한다.
...
1. **문항 리라이팅** — 번역투를 걷어낸 자연스러운 한국어(`text_ko_friendly`)로 다시 쓴다. 규칙·도구는 [`reference/rewriting/`](reference/rewriting/README.md)(+`review.py`), 원칙 예시는 `raw/korean/plain-language-examples.json`. 사람이 직접 확정하는 웹 어노테이터는 [`../tools/annotator/`](../tools/annotator/README.md)(`python3 tools/annotator/server.py`) — `translations/ko/` 15개 파일 전체 대상, 원문/번역/친화 비교 후 선택·직접입력. 상업까지 안전한 원본은 **퍼블릭 도메인 IPIP 계열**(`raw/bigfive/`, `raw/mbti/items.ipip-bffm.json`).
