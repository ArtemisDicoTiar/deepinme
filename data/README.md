...
1. **문항 리라이팅** — 번역투를 걷어낸 자연스러운 한국어(`text_ko_friendly`)로 다시 쓴다. 규칙·도구는 [`reference/rewriting/`](reference/rewriting/README.md)(+`review.py`), 원칙 예시는 `raw/korean/plain-language-examples.json`. 상업까지 안전한 원본은 **퍼블릭 도메인 IPIP 계열**(`raw/bigfive/`, `raw/mbti/items.ipip-bffm.json`).
